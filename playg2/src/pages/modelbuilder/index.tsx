...
const Demo: React.FC<IProps> = (demoProps: IProps) => {
  ...
  /** 连线source数据 */
  const [relationSourceData, setRelationSourceData] = useState<EntityCanvasModel>()
  /** 连线target数据 */
  const [relationTargetData, setRelationTargetData] = useState<EntityCanvasModel>()
  ...
  /** 设置鼠标样式 */
  const configCursorTip = ({ left, top, display } : { left:string, top:string, display:string }) => {
  ...
  >
    ...
    >
      <XFlow className="xflow-er-solution-container" commandConfig={cmdConfig} onLoad={onLoad}>
        ...
        />
        <XFlowCanvas className="xflow-insights-container" config={graphConfig}>
